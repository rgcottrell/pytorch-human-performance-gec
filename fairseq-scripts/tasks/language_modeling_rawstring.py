...
from data.indexed_raw_string_dataset import IndexedRawStringDataset

@register_task('language_modeling_rawstring')
class LanguageModelingRawTask(FairseqTask):
    """
    ...
    def load_dataset(self, split, line, combine=False):
        """Load a given line into dataset split.

        Args:
            split (str): name of the split (e.g., train, valid, test)
            line (str): name of the split (e.g., train, valid, test)
        """

        loaded_datasets = []

        ds = IndexedRawStringDataset(line, self.dictionary)
        ...
        print('| {} {} examples'.format(self.args.data, len(loaded_datasets[-1])))

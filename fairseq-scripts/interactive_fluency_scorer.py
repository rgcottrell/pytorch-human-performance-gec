...
from tasks import language_modeling_rawstring


class InteractiveFluencyArgs(dict):
    """Encapsulate args to build FluencyScorer."""

    def __init__(self, path, data):
        self['path'] = path
        self['data'] = data
        # Set options to allow raw string / line.
        self['task'] = 'language_modeling_rawstring'
        self['raw_text'] = True
...
class InteractiveFluencyScorer(object):
    ...
    def __init__(self, path, data, use_cpu=True):
        ...
        self.args = InteractiveFluencyArgs(path, data)
    ...
    def run(self, subset, line):
        # Load the dataset.
        self.task.load_dataset(subset, line)
